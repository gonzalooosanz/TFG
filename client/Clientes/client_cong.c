...
void on_delivered(char * payload){
	...
	time_t timestamp;
	char src;
	sscanf(payload, "%c,%d,%ld",&src, &id, &timestamp);
...
int main() {
	...
	// {
		...
		char name_file[40];
		sprintf(name_file, "Tam_pack_%dm_QOS_%d_TAMPACK_%d.txt",DISTANCE, j, (int)TAMPACKAGES);
		...
		for(int i=0; i<=(int)NUMPACKAGES; i++) {
			...
			sprintf(mess, "%c, %d,%ld, %s", source, i, cur_time.tv_sec*1000000 + cur_time.tv_usec, increase_load);
